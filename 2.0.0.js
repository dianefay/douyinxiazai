...
// @version      2.0.0
// @description  下载抖音用户主页数据! 浮动UI工具箱+多视频批量下载+最高清选择+下载历史+任务管理
// @author       xxmdmst
...
(function () {
    ...
    GM_registerMenuCommand("📊 下载作品数据(GBK)", () => downloadData(null, "gbk"));
    GM_registerMenuCommand("👤 下载作者数据(UTF8)", () => downloadUserData(null));
    GM_registerMenuCommand("👤 下载作者数据(GBK)", () => downloadUserData(null, "gbk"));
    ...
    // ========== 下载核心 ==========
    const downloadUrl = (url, node, filename, ext, awemeId) => {
        if (downloadHistory.some(x => x.awemeId === awemeId)) {
            toast("已下载过，跳过");
            node.textContent = "✅已下载"; node.style.color = "green";
            setTimeout(() => { node.textContent = "下载视频"; node.style.color = ""; }, 2000);
            return;
        }
        if (isPaused) { toast("已暂停"); return; }
        toast("开始下载");
        ...
        currentXHR.onload = () => {
            ...
            updateTaskStatus();
            if (currentDownloadIndex < downloadQueue.length && !isPaused) setTimeout(downloadNextInQueue, 500);
        ...
        currentXHR.onabort = () => { toast("已暂停"); node.textContent = "已暂停"; };
        currentXHR.send();
    };
    ...
    const downloadVideo = (aweme, node) => {
        ...
        downloadUrl(aweme.url, node, getAwemeName(aweme), aweme.images ? "mp3" : "mp4", id);
    };

    // ========== 任务管理 ==========
    function initDownloadQueue() {
        downloadQueue = Array.from(window.batchDownloadVideoIds);
        currentDownloadIndex = 0; isPaused = false; downloadProgress = {};
    ...
    function downloadNextInQueue() {
        if (isPaused || currentDownloadIndex >= downloadQueue.length) return;
        let id = downloadQueue[currentDownloadIndex];
        let aweme = window.all_aweme_map.get(id);
        if (!aweme) { currentDownloadIndex++; GM_setValue("currentDownloadIndex", currentDownloadIndex); downloadNextInQueue(); return; }
        let node = { textContent: `下载${currentDownloadIndex + 1}/${downloadQueue.length}`, style: {} };
        downloadVideo(aweme, node);
    }

    function pauseDownload() {
        if (isPaused) return;
        isPaused = true; GM_setValue("isPaused", true);
        if (currentXHR) { currentXHR.abort(); currentXHR = null; }
        toast("已暂停"); updateTaskStatus();
    }

    function resumeDownload() {
        if (!isPaused || downloadQueue.length === 0) return;
        isPaused = false; GM_setValue("isPaused", false);
        toast("继续下载"); downloadNextInQueue(); updateTaskStatus();
    }

    function resetDownloadQueue() {
        downloadQueue = []; currentDownloadIndex = 0; isPaused = false; downloadProgress = {};
        GM_setValue("downloadQueue", []); GM_setValue("currentDownloadIndex", 0);
        GM_setValue("isPaused", false); GM_setValue("downloadProgress", {});
        currentXHR = null; toast("队列已重置"); updateTaskStatus();
    }

    function updateTaskStatus() {
        if (window.taskEl) {
            let s = "📥 任务状态：";
            if (downloadQueue.length === 0) s += "无任务";
            else {
                s += isPaused ? "⏸️已暂停" : "▶️下载中";
    ...
    function batchDownloadVideos(btn) {
        if (window.batchDownloadVideoIds.size === 0) { toast("未选择视频"); return; }
        if (isPaused) { resumeDownload(); return; }
        btn.disabled = true; initDownloadQueue();
        toast(`开始下载 ${downloadQueue.length} 个`);
        downloadNextInQueue();
    }
    ...
    function markAsDownloaded() {
        ...
        ids.forEach(id => {
            ...
            if (a) {
                addToDownloadHistory(id, a.nickname || "未知", getAwemeName(a) + ".mp4");
            }
        });
    ...
    function showDownloadHistory() {
        ...
        html += `<table style='width:100%;border-collapse:collapse;margin-top:10px;'>
            <tr style='background:#334155;text-align:left'>
                ...
                <th style='padding:10px;border:1px solid #475569'>视频ID</th>
    ...
    function refreshBatchCount() {
        if (window.countEl) window.countEl.textContent = `已选：${window.batchDownloadVideoIds.size} 个`;
        if (window.dlBtn) window.dlBtn.textContent = `下载已选[${window.batchDownloadVideoIds.size}]个视频`;
    }
    ...
    function selectAllCurrentUp() {
        ...
        refreshBatchCount(); toast(`已全选 ${list.length} 个`);
    }
    ...
    function createToolBox() {
        ...
        dlBtn.textContent = "下载已选[0]个视频";
        dlBtn.onclick = () => batchDownloadVideos(dlBtn);
        window.dlBtn = dlBtn;
        ...
        pauseBtn.style.cssText = btnStyle + "background:#f59e0b;";
        pauseBtn.onclick = pauseDownload;
        let resumeBtn = document.createElement('button');
        resumeBtn.textContent = "继续";
        resumeBtn.style.cssText = btnStyle + "background:#059669;";
        resumeBtn.onclick = resumeDownload;
        let resetBtn = document.createElement('button');
        resetBtn.textContent = "重置";
        resetBtn.style.cssText = btnStyle + "background:#dc2626;";
        resetBtn.onclick = resetDownloadQueue;
        line5.append(pauseBtn, resumeBtn, resetBtn);
        body.appendChild(line5);

        let countEl = document.createElement('div');
        countEl.style.cssText = "text-align:center; font-size:12px; color:#94a3b8; margin-top:8px;";
        countEl.textContent = "已选：0 个";
        window.countEl = countEl;
        body.appendChild(countEl);

        let copyBtn = document.createElement('button');
        copyBtn.style.cssText = "width:100%; padding:10px; background:#14b8a6; color:#fff; border:none; border-radius:8px; font-weight:bold; margin-top:10px;";
        copyBtn.textContent = "复制当前作者信息";
        copyBtn.onclick = () => copyUserData(copyBtn);
