...
// @version      2.1.1
// @description  下载抖音用户主页数据! 自动过滤已下载作品，多线程批量下载，作品数量进度，超时重试，打包防崩，失败统计，（多线程+自动跳过已下载+作品数进度）
// @author       xxmdmst
...
(function () {
    ...
    // 多线程批量下载相关
    let maxConcurrent = 5;            // 固定5线程
    let activeDownloads = 0;          // 当前活跃下载数
    let pendingWorks = [];             // 待下载的作品列表（对象），按作品计数
    let totalWorkCount = 0;            // 总作品数量（过滤后）
    let completedWorkCount = 0;        // 已完成下载的作品数量
    let failedAwemeIds = new Set();     // 记录失败的作品ID（不标记下载历史）
    let successWorks = [];              // 存储成功下载的作品信息，用于打包 (每个作品可能产生多个文件)
    let isDownloadingBatch = false;     // 是否处于批量下载中
    let abortControllers = new Map();    // 存储每个下载任务的AbortController
    // 打包分批阈值
    ...
    async function fetchWithRetry(url, signal, retries = 3, timeout = 3000) {
        ...
        for (let i = 0; i <= retries; i++) {
            ...
            try {
                ...
                lastError = err;
                if (i < retries) {
                    await new Promise(r => setTimeout(r, 500)); // 重试间隔500ms
                }
            }
        }
        throw lastError;
    ...
    async function startBatchDownload() {
        ...
        // 所有下载完成后（无论成功失败）进入打包阶段, 但需检查是否被暂停
        if (!isBatchPaused) {
            await packageAllSuccessFiles();
        } else {
            toast("批量下载已暂停，未进入打包");
        }
        isDownloadingBatch = false;
    ...
    // 打包所有成功文件（支持分批，支持暂停）
    async function packageAllSuccessFiles() {
        ...
        isPackaging = true;
        updateTaskStatus();
        const totalFiles = successWorks.length;
        if (totalFiles > BATCH_ZIP_SIZE) {
            const batchCount = Math.ceil(totalFiles / BATCH_ZIP_SIZE);
            for (let batchIdx = 0; batchIdx < batchCount; batchIdx++) {
                ...
                const end = Math.min(start + BATCH_ZIP_SIZE, totalFiles);
                const batchBlobs = successWorks.slice(start, end);
                await generateZip(batchBlobs, batchIdx + 1, batchCount);
                updateTaskStatus();
            }
        } else {
            await generateZip(successWorks, 1, 1);
        }
        // 标记下载历史（按作品去重，避免重复标记）
        const uniqueSuccessWorks = new Map(); // awemeId -> {nickname, filename example}
    ...
    async function generateZip(blobsList, partNum, totalParts) {
        const zip = new JSZip();
        for (let item of blobsList) {
        ...
        if (totalParts === 1) {
            ...
            zipFilename = `${authorNick}_抖音批量下载_第${partNum}部分_${timeFormat(null, 'yyyyMMddHHmmss')}.zip`;
        }
        toast(`正在生成压缩包${partNum}/${totalParts}: ${zipFilename}`);
        const zipBlob = await zip.generateAsync({ type: "blob" });
        createDownloadLink(zipBlob, zipFilename, "zip", "", "batch_" + Date.now() + "_" + partNum);
    }
    ...
    function updateTaskStatus() {
        if (window.taskEl) {
            let s = "";
            if (isPackaging) {
                const packedFiles = successWorks.length;
                const totalFiles = successWorks.length; // 这里显示文件数也可以
                s = `📦 正在打包：已打包 ${packedFiles} / ${totalFiles} 个文件`;
            } else if (isDownloadingBatch) {
                s = `⏬ 下载中 | 已完成 ${completedWorkCount} / ${totalWorkCount} 作品 | 并发: ${activeDownloads}/${maxConcurrent}`;
    ...
    function updateFailureStats(successCount, failCount) {
        ...
        if (failureStatsDiv) {
            if (failCount > 0) {
                failureStatsDiv.innerHTML = `⚠️ 打包完成：成功 <span style="color:#4ade80;">${successCount}</span> 个作品，失败 <span style="color:#ff5555;">${failCount}</span> 个（失败作品可重新选择下载）`;
                failureStatsDiv.style.display = "block";
            } else {
                failureStatsDiv.innerHTML = `✅ 打包完成：成功 ${successCount} 个作品，无失败`;
                failureStatsDiv.style.display = "block";
            }
        }
    }
    ...
    function clearFailureStatsUI() {
        if (failureStatsDiv) {
            ...
            failureStatsDiv.style.display = "none";
        }
    ...
    function showDownloadHistory() {
        ...
        box.style.cssText = `position:fixed; top:50%; left:50%; transform:translate(-50%,-50%); width:750px; max-height:80vh; background:#1e293b; color:#fff; padding:20px; border-radius:12px; z-index:9999999; overflow-y:auto; box-shadow:0 8px 32px #0008;`;
        ...
        let html = `<div style='display:flex;justify-content:space-between;align-items:center;'><h3>📥 下载历史记录</h3></div>`;
        html += `<table style='width:100%;border-collapse:collapse;margin-top:10px;'><tr style='background:#334155;text-align:left'><th style='padding:10px;border:1px solid #475569'>UP主昵称</th><th style='padding:10px;border:1px solid #475569'>作品ID</th><th style='padding:10px;border:1px solid #475569'>文件名</th></table>`;
        let allText = "下载历史：\n";
        downloadHistory.forEach((item, i) => {
            html += `<tr style='border-bottom:1px solid #475569'><td style='padding:10px;border:1px solid #475569'>${item.nickname}</td><td style='padding:10px;border:1px solid #475569'>${item.awemeId}</td><td style='padding:10px;border:1px solid #475569'>${item.filename}</td></tr>`;
    ...
    function reverseSelect() {
        ...
        document.querySelectorAll('[data-vid]').forEach(n => {
            let b = Array.from(n.children).find(x => x.textContent.includes("选择"));
            if (b) { let has = window.batchDownloadVideoIds.has(n.dataset.vid); b.textContent = has ? "✅已选择" : "❌未选择"; b.style.background = has ? "#ff4444" : ""; b.style.color = has ? "#fff" : ""; }
        });
        toast("已反选");
        refreshBatchCount();
        clearFailureStatsUI();
    ...
    function downloadImage(aweme, btn) {
        if (downloadHistory.some(x => x.awemeId === aweme.awemeId)) { toast("已下载"); btn.textContent = "✅已下载"; btn.style.color = "green"; setTimeout(() => { btn.textContent = "图片打包"; btn.style.color = ""; }, 2000); return; }
        let zip = new JSZip();
        let old = btn.textContent; btn.textContent = "打包中...";
        Promise.all(aweme.images.map((u, i) => fetch(u).then(r => r.arrayBuffer()).then(b => zip.file(`img${i+1}.jpg`, b)))).then(() => zip.generateAsync({ type: "blob" })).then(b => { createDownloadLink(b, getAwemeName(aweme), "zip", "【图文】", aweme.awemeId); btn.textContent = old; }).catch(() => { btn.textContent = old; toast("打包失败"); });
    }
    ...
    function downloadData(node, enc) {
        ...
        Array.from(window.all_aweme_map.values()).sort((a,b)=>b.create_time-a.create_time).forEach(a=>{ t+= [a.nickname,`"${(a.desc||"").replace(/"/g,'""')}"`,"https://www.douyin.com/video/"+a.awemeId,a.diggCount,a.commentCount,a.collectCount,a.shareCount,a.date,a.duration,a.tag,a.video_tag,a.cover,`"${a.url}"`].join(",")+"\n"; });
    ...
    function downloadUserData(node, enc) {
        ...
        Array.from(window.user_map.values()).sort((a,b)=>b.create_time-a.create_time).forEach(u=>{ t+= [u.nickname,u.following_count,u.mplatform_followers_count,u.total_favorited,u.unique_id,u.ip_location,u.gender,u.city,`"${(u.signature||"").replace(/"/g,'""')}"`,u.aweme_count,timeFormat(u.create_time),"https://www.douyin.com/user/"+u.uid].join(",")+"\n"; });
        if (enc === "gbk") t = str2gbk(t);
    ...
    function downloadVideo(aweme, node, isBatch = false) {
        ...
        if (downloadHistory.some(x => x.awemeId === id)) { toast("已下载"); node.textContent = "✅已下载"; node.style.color = "green"; setTimeout(()=>{ node.textContent = isBatch?`下载${currentDownloadIndex+1}/${downloadQueue.length}`:"下载视频"; node.style.color=""; },2000); return; }
    ...
    function downloadUrl(url, node, filename, ext, awemeId, isBatch = false) {
        if (downloadHistory.some(x => x.awemeId === awemeId)) { toast("已下载过，跳过"); if(isBatch){/*兼容单次逻辑*/} return; }
        toast("开始下载");
        currentXHR = new XMLHttpRequest();
        currentXHR.open('GET', url.replace('http://','https://'), true);
        currentXHR.responseType = 'blob';
        let oldText = node.textContent;
        currentXHR.onload = () => { createDownloadLink(currentXHR.response, filename, ext, "", awemeId); node.textContent = oldText; };
    ...
    function createToolBox() {
        let box = document.createElement('div');
        box.id = "dy-toolbox";
        box.style.cssText = `position:fixed; right:20px; top:100px; width:260px; background:#1e293b; border-radius:12px; z-index:999999; color:#fff; box-shadow:0 8px 32px #0004; cursor:move; overflow:hidden; border:1px solid #334155;`;
        ...
        let title = document.createElement('span'); title.textContent = "工具箱"; title.style.fontWeight = "bold";
        let toggle = document.createElement('button'); toggle.textContent = "收起"; toggle.style.cssText = "background:#38bdf8; color:#fff; border:none; border-radius:6px; padding:4px 10px; font-size:12px; cursor:pointer;";
        ...
        failureStatsDiv.style.cssText = "background:#1e293b; padding:6px; border-radius:6px; text-align:center; font-size:12px; margin-bottom:8px; color:#ffb347; display:none;";
        ...
        document.body.appendChild(box);
        let fold = false;
        toggle.onclick = () => { fold = !fold; body.style.display = fold ? "none" : "block"; toggle.textContent = fold ? "展开" : "收起"; box.style.width = fold ? "100px" : "260px"; };
    ...
    window.addEventListener('load', () => {
        ...
        toast("抖音下载工具已加载完成（自动跳过已下载，作品数进度）");
    });
})();