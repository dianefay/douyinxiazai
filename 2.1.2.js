...
// @version      2.1.2
// @description  下载抖音用户主页数据! 自动过滤已下载作品，多线程批量下载，作品数量进度，超时重试，打包防崩，失败统计（顶部工具栏+历史表格优化+文件名修正）
// @author       xxmdmst
...
(function () {
    ...
    function addToDownloadHistory(awemeId, nickname, filename) {
        ...
        downloadHistory.push({
            ...
            filename: filename   // 此处确保传入的就是视频/图片名，不是压缩包名
        });
    ...
    function createDownloadLink(blob, filename, ext, prefix, awemeId) {
        ...
        if (awemeId && !awemeId.startsWith("batch_")) {
            ...
            // 核心：保证历史记录中的文件名是作品原始文件名，而不是压缩包名
    ...
    async function packageAllSuccessFiles() {
        ...
        // 标记下载历史：每个成功作品的文件名是原始视频/图片名，不是压缩包名
    ...
    function createToolBox() {
        ...
        // 折叠逻辑：不改变盒子宽度，只隐藏/显示 body，并调整标题按钮文本
        let isFold = false;
        toggleBtn.onclick = () => {
            isFold = !isFold;
            body.style.display = isFold ? "none" : "block";
            toggleBtn.textContent = isFold ? "展开" : "收起";
            // 保持宽度不变，标题和按钮在同一行，自然美观
        };

        // 拖拽移动（已经使用 top/left 定位）
        ...
        document.onmousemove = (e) => {
            ...
            box.style.transform = "none";  // 清除居中变换，改为绝对定位
        };
        document.onmouseup = () => {
    ...
    window.addEventListener('load', () => {
        ...
        toast("抖音下载工具已加载 (v2.1.2 工具箱居中+表格美化+文件名修正)");
    });
})();