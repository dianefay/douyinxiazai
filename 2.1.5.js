...
// @version      2.1.5
// @description  下载抖音用户主页数据! 自动过滤已下载作品，多线程批量下载，作品数量进度，超时重试，打包防崩，失败统计（顶部工具栏+历史表格优化+文件名修正）| 新增: 实时状态栏显示下载/打包进度百分比，下方详情栏动态显示当前下载文件名/打包进度条 | 修复: 暂停/恢复支持，重置功能强制清除所有任务 | 新增: 打包阈值/始终打包选项，可配置并发数、ZIP大小、重试次数、超时时间(⚙️设置)
// @author       xxmdmst
