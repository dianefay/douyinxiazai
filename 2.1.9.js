// ==UserScript==
// @name         抖音下载工具-调试版Pro218
// @namespace    http://tampermonkey.net/
// @version      2.2.0
// @description  下载抖音用户主页数据! 自动过滤已下载作品，多线程批量下载，作品数量进度，超时重试，打包防崩，失败统计（顶部工具栏+历史表格优化+文件名修正）| 新增: 实时状态栏显示下载/打包进度百分比，下方详情栏动态显示当前下载文件名/打包进度条 | 修复: 暂停/恢复支持，重置功能强制清除所有任务 | 新增: 打包阈值/始终打包选项，可配置并发数、ZIP大小、重试次数、超时时间(⚙️设置) | 优化工具栏：默认收起小圆条，自动定位搜索框右侧（带间距），拖拽后暂停跟随，双击头部恢复吸附 | 新增: 允许重新下载已下载作品（忽略下载记录）
// @author       xxmdmst
...
(function () {
    ...
    const SETTINGS_DEFAULTS = {
        ...
        maxRetries: 3,
        fetchTimeout: 3000,
        toolbarExpanded: false,      // 工具栏默认展开
        scanInterval: 3000,          // 周期补扫间隔(毫秒)，0 表示关闭
        toastDuration: 3000,         // 消息提示显示时长(毫秒)
        allowRedownload: false       // 允许重新下载（忽略下载记录）
    };
    ...
    // ========== 下载历史管理（修复多标签页同步） ==========
    let downloadHistory = []; // 不再在初始化时直接赋值，而是通过同步函数获取

    // 从GM存储同步最新历史到全局变量，并返回
    function syncDownloadHistory() {
        downloadHistory = GM_getValue("downloadHistory", []);
        return downloadHistory;
    }

    // 保存历史到GM存储（同时更新全局变量）
    function setDownloadHistory(newHistory) {
    ...
    function addToDownloadHistory(awemeId, nickname, filename) {
        let history = syncDownloadHistory(); // 先获取最新列表
        // 如果允许重新下载，依然添加记录，但不去重（方便查看多次下载）
        if (!currentSettings.allowRedownload) {
            history = history.filter(item => item.awemeId !== awemeId);
        }
        history.push({
            awemeId: awemeId,
    ...
    function markAsDownloaded() {
        ...
        let history = syncDownloadHistory();
        ids.forEach(id => {
            let a = window.all_aweme_map.get(id);
            if (a) {
                let filename = getAwemeName(a) + (a.images ? "_1.jpg" : ".mp4");
                if (!currentSettings.allowRedownload) {
                    history = history.filter(item => item.awemeId !== id);
                }
                history.push({
                    awemeId: id,
                    nickname: a.nickname || "未知",
                    filename: filename
                });
            }
        });
    ...
    function unmarkAsDownloaded() {
        ...
        if (ids.length === 0) { toast("未选择"); return; }
        let history = syncDownloadHistory();
        history = history.filter(x => !ids.includes(x.awemeId));
        setDownloadHistory(history);
        toast(`已取消标记`);
        refreshAllDownloadStatus();
        clearFailureStatsUI();
    ...
    function showDownloadHistory() {
        ...
        history.forEach((item, i) => {
            ...
            html += `<tr style='background:${bg};border-bottom:1px solid #475569'>
                <td style='padding:10px;border:1px solid #475569'>${item.nickname} \u{79}
                <td style='padding:10px;border:1px solid #475569'>${item.awemeId} \u{79}
                <td style='padding:10px;border:1px solid #475569'>${escapeHtml(item.filename)} \u{79}
              \u{79}`;
    ...
    let packagedProcessedFiles = 0;
    let totalPackFiles = 0;
    let currentPackagingIndex = 0;
    let isPackaging = false;
    let isIndividualSaving = false;
    ...
    const toast = (msg, duration) => {
        if (duration === undefined) duration = loadSettings().toastDuration;
    ...
    async function saveWorksIndividually() {
        ...
        // 注意：addToDownloadHistory 内部已经同步历史并刷新UI，无需再单独处理
        return true;
    }
    ...
    async function startBatchDownload() {
        ...
        let pendingIds;
        if (currentSettings.allowRedownload) {
            // 允许重新下载：不过滤，直接下载所有选中的
            pendingIds = selectedIds;
            toast(`开始下载 ${pendingIds.length} 个作品（允许重新下载）`);
        } else {
            // 正常模式：过滤已下载
            const history = syncDownloadHistory();
            const downloadedIds = new Set(history.map(item => item.awemeId));
            pendingIds = selectedIds.filter(id => !downloadedIds.has(id));
            if (pendingIds.length === 0) {
                toast("所选作品均已下载，如需重新下载请开启设置中的「允许重新下载」");
                return;
            }
    ...
    async function scheduleDownloads() {
        const maxConc = currentSettings.maxConcurrent;
    ...
    function refreshAllDownloadStatus() {
        syncDownloadHistory(); // 确保全局历史与存储一致
    ...
    function updatePauseResumeBtnText() {
        if (window.pauseResumeBtn) {
            const hasActiveTask = isDownloadingBatch || isPackaging || isIndividualSaving;
            if (hasActiveTask) {
                window.pauseResumeBtn.textContent = isBatchPaused ? "继续" : "暂停";
                window.pauseResumeBtn.style.background = "#f59e0b";
                window.pauseResumeBtn.style.opacity = "1";
            } else {
                window.pauseResumeBtn.textContent = "暂停/继续";
                window.pauseResumeBtn.style.background = "#475569";
    ...
    function showSettingsPanel() {
        ...
        panel.innerHTML = `
            ...
            <div style="margin-bottom:12px;">
                <label style="display:flex; align-items:center; gap:8px; margin-bottom:8px;">
                    <input type="checkbox" id="allowRedownload" ${settings.allowRedownload ? 'checked' : ''}>
                    <span>允许重新下载（忽略下载记录）</span>
                </label>
                <div style="font-size:12px; color:#94a3b8; margin-top:2px;">开启后，已下载的作品可以再次下载，且按钮不再显示“已下载”</div>
            </div>
            <div style="margin-bottom:12px;">
            ...
            <div style="margin-bottom:12px; margin-top:12px;">
                <label style="display:flex; align-items:center; gap:8px; margin-bottom:8px;">
                    <input type="checkbox" id="toolbarExpanded" ${settings.toolbarExpanded ? 'checked' : ''}>
                    <span>工具栏默认展开</span>
                </label>
                <div style="display:flex; align-items:center; gap:8px; margin-bottom:6px;">
                    <span style="width:100px;">补扫间隔(ms):</span>
                    <input type="number" id="scanInterval" value="${settings.scanInterval}" min="0" step="500" style="flex:1; background:#0f172a; color:white; border:1px solid #475569; border-radius:6px; padding:6px;">
                    <span style="font-size:12px;">(0=关闭)</span>
                </div>
                <div style="display:flex; align-items:center; gap:8px;">
                    <span style="width:100px;">提示时长(ms):</span>
                    <input type="number" id="toastDuration" value="${settings.toastDuration}" min="500" step="500" style="flex:1; background:#0f172a; color:white; border:1px solid #475569; border-radius:6px; padding:6px;">
                </div>
            </div>
        ...
        panel.querySelector('#settingsSave').onclick = () => {
            const newSettings = {
                ...
                toolbarExpanded: panel.querySelector('#toolbarExpanded').checked,
                scanInterval: parseInt(panel.querySelector('#scanInterval').value, 10) || 0,
                toastDuration: parseInt(panel.querySelector('#toastDuration').value, 10) || 3000,
                allowRedownload: panel.querySelector('#allowRedownload').checked
            };
    ...
    function downloadImage(aweme, btn) {
        // 允许重新下载时跳过已下载检查
        if (!currentSettings.allowRedownload && downloadHistory.some(x => x.awemeId === aweme.awemeId)) {
    ...
    function downloadVideo(aweme, node, isBatch = false) {
        ...
        // 允许重新下载时跳过已下载检查
        if (!currentSettings.allowRedownload && downloadHistory.some(x => x.awemeId === id)) {
        ...
        if (aweme.images) {
            downloadImage(aweme, node);
            return;
        }
        downloadUrl(aweme.url, node, getAwemeName(aweme), "mp4", id, isBatch);
    }

    function downloadUrl(url, node, filename, ext, awemeId, isBatch = false) {
        // 允许重新下载时跳过已下载检查
        if (!currentSettings.allowRedownload && downloadHistory.some(x => x.awemeId === awemeId)) {
            toast("已下载过，跳过");
            return;
        }
        toast("开始下载");
    ...
    function startQuickScan() {
        stopQuickScan();
        let noUnprocessedCount = 0;
        quickScanTimer = setInterval(() => {
            const unprocessed = document.querySelectorAll(
                'a[href*="/video/"]:not([data-vid]), a[href*="/note/"]:not([data-vid])'
            );
            if (unprocessed.length > 0) {
                unprocessed.forEach(createButtonGroup);
                noUnprocessedCount = 0;
            } else {
                noUnprocessedCount++;
                if (noUnprocessedCount >= 4) {
                    stopQuickScan();
                }
            }
        }, 500);
    ...
    function stopCycleScan() {
        if (cycleScanTimer) {
            clearInterval(cycleScanTimer);
            cycleScanTimer = null;
        }
    }

    function startCycleScan() {
        stopCycleScan();
        const interval = loadSettings().scanInterval;
        if (interval > 0) {
            cycleScanTimer = setInterval(() => {
                document.querySelectorAll('a[href*="/video/"]:not([data-vid]), a[href*="/note/"]:not([data-vid])').forEach(createButtonGroup);
            }, interval);
        }
    }

    // ========== 公共数据处理函数 ==========
    function processInterceptedData(pathname, data) {
        if (pathname.startsWith("/aweme/v1/web/user/profile/other")) {
            let u = data.user;
            if (!u) return;
            let user = {
                uid: u.uid,
                ...
                mplatform_followers_count: u.mplatform_followers_count,
                total_favorited: u.total_favorited,
                unique_id: u.unique_id || u.short_id,
                ip_location: (u.ip_location || "").replace("IP属地：", ""),
                gender: u.gender ? "男女"[u.gender - 1] || "" : "",
                city: [u.province, u.city, u.district].filter(x => x).join("·"),
            ...
            localStorage.setItem('user_local_data', JSON.stringify(
                Array.from(user_map.values()).sort((a, b) => b.create_time - a.create_time).slice(0, max_author_num)
            ));
        } else {
            let list = [];
            if (data.aweme_list) list = data.aweme_list;
            else if (data.data && data.data.length) list = data.data.map(x => x.aweme || x.aweme_info).filter(x => x);
            if (list.length) {
                list.forEach(item => {
                    let a = {
                        ...
                        desc: item.desc,
                        date: timeFormat(item.create_time),
                ...
                refreshAllDownloadStatus();
                // 补扫一次按钮，同时启动快速巡检
                document.querySelectorAll('a[href*="/video/"],a[href*="/note/"]').forEach(createButtonGroup);
                startQuickScan();
            }
        }
    }
    ...
    function interceptResponse() {
        ...
        window.fetch = async function (...args) {
            ...
            if (p.startsWith("/aweme/v1/web/")) {
                try {
                    const clonedResponse = response.clone();
                    const data = await clonedResponse.json();
                    processInterceptedData(p, data);
                } catch (e) {
                    console.error("fetch 拦截解析失败", e);
                }
            }
            return response;
        };
    ...
    function positionToSearchBox() {
        if (!toolbarBox || !autoPositionEnabled) return;
        const searchBox = findSearchInput();
        if (!searchBox) {
            toolbarBox.style.left = '50%';
            toolbarBox.style.transform = 'translateX(-50%)';
            toolbarBox.style.top = '20px';
            toolbarBox.style.right = 'auto';
            return;
        }
        const rect = searchBox.getBoundingClientRect();
        const boxWidth = toolbarBox.offsetWidth;
        let leftPos = rect.right + HORIZONTAL_OFFSET;
        let topPos = rect.top + (rect.height / 2) - (toolbarBox.offsetHeight / 2);
        const maxLeft = window.innerWidth - boxWidth - 8;
        if (leftPos > maxLeft) leftPos = maxLeft;
        if (leftPos < 8) leftPos = 8;
        if (topPos < 8) topPos = 8;
        if (topPos + toolbarBox.offsetHeight > window.innerHeight - 8) {
            topPos = window.innerHeight - toolbarBox.offsetHeight - 8;
        }
        toolbarBox.style.left = leftPos + 'px';
        toolbarBox.style.top = topPos + 'px';
        toolbarBox.style.right = 'auto';
        toolbarBox.style.transform = 'none';
    }

    function bindAutoPosition() {
        if (scrollListener) window.removeEventListener('scroll', scrollListener);
        if (resizeObserver) resizeObserver.disconnect();
        scrollListener = () => { if (autoPositionEnabled && toolbarBox) positionToSearchBox(); };
        window.addEventListener('scroll', scrollListener, { passive: true });
        resizeObserver = new ResizeObserver(() => { if (autoPositionEnabled && toolbarBox) positionToSearchBox(); });
        if (toolbarBox) resizeObserver.observe(document.body);
        if (toolbarBox) positionToSearchBox();
    }

    function refreshLayout() {
        if (!toolbarBox) return;
        const headDiv = toolbarBox.querySelector('.toolbar-head');
        const bodyDiv = toolbarBox.querySelector('.toolbar-body');
        if (!headDiv || !bodyDiv) return;

        if (isExpanded) {
            toolbarBox.style.width = '260px';
            bodyDiv.style.display = 'block';
            headDiv.innerHTML = '<span style="font-weight:bold; white-space:nowrap;">🛠️ 工具栏</span><button id="toggleExpandBtn" style="background:#38bdf8; color:#fff; border:none; border-radius:6px; padding:4px 10px; font-size:12px; cursor:pointer; white-space:nowrap;">收起</button>';
        } else {
            toolbarBox.style.width = '70px';
            bodyDiv.style.display = 'none';
            headDiv.innerHTML = '<span style="font-weight:bold; white-space:nowrap;">⚙️</span><button id="toggleExpandBtn" style="background:#38bdf8; color:#fff; border:none; border-radius:6px; padding:4px 8px; font-size:11px; cursor:pointer; white-space:nowrap;">展开</button>';
        }
        const expandBtn = toolbarBox.querySelector('#toggleExpandBtn');
        if (expandBtn) {
            expandBtn.onclick = (e) => {
                e.stopPropagation();
                isExpanded = !isExpanded;
                refreshLayout();
                if (autoPositionEnabled) positionToSearchBox();
            };
        }
        if (autoPositionEnabled) positionToSearchBox();
    }
    ...
    function createToolBox() {
        if (toolbarBox && document.body.contains(toolbarBox)) toolbarBox.remove();
        toolbarBox = document.createElement('div');
        toolbarBox.id = "dy-toolbox";
        ...
        head.style.cssText = "padding:8px 12px; display:flex; justify-content:space-between; align-items:center; background:#273444; cursor: move; user-select:none;";
        toolbarBox.appendChild(head);

        let body = document.createElement('div');
        body.className = 'toolbar-body';
        body.style.padding = "12px";
        body.style.display = "none";
        toolbarBox.appendChild(body);
        ...
        dlBtn.style.cssText = "width:100%; padding:10px; background:#f97316; color:#fff; border:none; border-radius:8px; font-weight:bold; margin-bottom:8px; cursor:pointer;";
        ...
        let mark = document.createElement('button'); mark.textContent = "✅ 标记下载"; mark.style.cssText = "flex:1; padding:8px; background:#10b981; color:#fff; border:none; border-radius:8px; font-weight:bold; cursor:pointer;"; mark.onclick = markAsDownloaded;
        let unmark = document.createElement('button'); unmark.textContent = "❌ 取消标记"; unmark.style.cssText = "flex:1; padding:8px; background:#ef4444; color:#fff; border:none; border-radius:8px; font-weight:bold; cursor:pointer;"; unmark.onclick = unmarkAsDownloaded;
        ...
        line6.style.gap = "6px";
        line6.style.marginTop = "8px";
        let manualScanBtn = document.createElement('button');
        manualScanBtn.textContent = "🔄 补扫按钮";
        manualScanBtn.style.cssText = "flex:1; padding:8px 0; border-radius:8px; border:none; color:#fff; cursor:pointer; font-size:14px; font-weight:bold; background:#6366f1;";
        manualScanBtn.onclick = () => {
            document.querySelectorAll('a[href*="/video/"],a[href*="/note/"]').forEach(createButtonGroup);
            startQuickScan();
            toast("已强制刷新按钮");
        };
        line6.appendChild(manualScanBtn);
        body.appendChild(line6);

        bindClearFailureOnButtons(body);
        document.body.appendChild(toolbarBox);

        isExpanded = loadSettings().toolbarExpanded || false;
        refreshLayout();
        ...
        const onDragStart = (e) => {
            ...
            startX = e.clientX;
            startY = e.clientY;
            startLeft = toolbarBox.offsetLeft;
            startTop = toolbarBox.offsetTop;
            toolbarBox.style.transition = "none";
            e.preventDefault();
        };
        const onDragMove = (e) => {
            if (!dragging) return;
            if (autoPositionEnabled) {
                ...
                toast("已脱离自动跟随，双击工具栏头部可重新吸附到搜索框", 2000);
            }
            let newLeft = startLeft + e.clientX - startX;
            let newTop = startTop + e.clientY - startY;
            newLeft = Math.min(Math.max(8, newLeft), window.innerWidth - toolbarBox.offsetWidth - 8);
            newTop = Math.min(Math.max(8, newTop), window.innerHeight - toolbarBox.offsetHeight - 8);
            toolbarBox.style.left = newLeft + "px";
            toolbarBox.style.top = newTop + "px";
            toolbarBox.style.right = "auto";
            toolbarBox.style.transform = "none";
        };
        const onDragEnd = () => {
            if (dragging) toolbarBox.style.transition = "width 0.2s ease";
            dragging = false;
        };
        ...
        head.addEventListener('dblclick', () => {
            ...
            positionToSearchBox();
            toast("已恢复自动跟随搜索框", 1500);
        });

        bindAutoPosition();
        const observer = new MutationObserver(() => {
            if (autoPositionEnabled && toolbarBox) positionToSearchBox();
        });
    ...
    function douyinVideoDownloader() {
        ...
        const clonePlayclarity2Download = (xgPlayer, videoId, videoContainer) => {
            ...
            function checkDatasetVid() {
                if (!toolDom.dataset.vid || toolDom.dataset.vid === "null") {
                    let activeVideo = player?.root?.closest('div[data-e2e="feed-active-video"]');
                    toolDom.dataset.vid = activeVideo ? activeVideo.getAttribute('data-e2e-vid') : null;
                }
            }
            ...
            downloadDom.onclick = () => {
                ...
                if (aweme && aweme.url) {
                    ...
                    let fallbackId = toolDom.dataset.vid;
                    downloadUrl(player.videoList[0].playAddr[0].src, downloadDom, filename, "mp4", fallbackId || null);
                } else {
                    toast('未捕获到对应数据源！')
                }
            };
